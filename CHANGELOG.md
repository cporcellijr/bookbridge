...
- **BookFusion highlight sync no longer deletes highlights off your other devices.** BookFusion reports a highlight timestamp that moves whenever BookBridge itself creates or edits the highlight on BookFusion, and the annotation hub was letting that unstable timestamp replace a highlight's identity key. KOReader devices identify highlights by their original creation time, so after one BookFusion sync cycle the device's next check-in looked like the highlight had been deleted — and the hub then deleted it from BookFusion, Readest, BookOrbit, and the other device, sometimes permanently (notes included). BookFusion now uses the same lossy-spoke rules as Grimmory: pulled data can update a highlight's note and color but can never rewrite its identity, unrecognized BookFusion copies re-attach to the existing highlight by matching text instead of duplicating it, and a truncated highlight listing can no longer be misread as a mass deletion.

- **Readest and Hardcover annotation sync no longer re-push everything every cycle.** Both spokes decided "has this changed?" by comparing timestamps that BookBridge's own bookkeeping writes kept moving, so every annotation was re-sent to Readest every 15 minutes (overwriting edits made in Readest before they could sync back) and Hardcover burned a rate-limited API call per book per cycle. Change detection now uses the hub's per-spoke version acknowledgments — the same mechanism devices use — so an annotation is only pushed when its content actually changed. Note and color edits made in Readest also now raise the annotation version, so they propagate to your KOReader devices instead of being silently absorbed.

- **BookFusion no longer opens synced books at the beginning.** Reading progress pushed to BookFusion previously set only the percentage, leaving BookFusion's own chapter/CFI anchor pointing at the book's start — so opening the book in BookFusion jumped to the start and then wrote that position back, undoing the sync in a loop. BookBridge now sends a real navigation anchor (chapter index, spine-normalized position, and a CFI) computed against BookFusion's own copy of the book, so it opens where you left off. (A one-time "unprocessable" warning right after uploading a new book to BookFusion is expected while BookFusion finishes ingesting the file; the next sync succeeds on its own.)

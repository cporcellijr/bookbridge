...
| Kobo Pull Behavior | `KOSYNC_WRITEBACK_MODE` | `generated_xpath` | `generated_xpath` sends bridge-generated positions, the default behavior, which can sometimes be incompatible with Kobo. `preserve_existing_locator` avoids Kobo jumping to page 1 when a generated position is incompatible; it updates percentage but keeps KOReader's last known position marker. |
